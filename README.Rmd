...
library](https://github.com/alugowski/fast_matrix_market) (version 1.7.6).
...
- **Extended Support**: `fastMatMR` supports standard R vectors, matrices,
  `Matrix` sparse objects, `spam` sparse matrices, and `SparseM` `matrix.csr`
  objects. Gzip-compressed `.mtx.gz` files are handled transparently.
...
accomplished in a hexb compatible design [using
